...
class AnswerSynthesize:
    ...
    def run(self, context: Dict[str, Any]) -> Dict[str, Any]:
        if context.get("graph_result_flag") == -1:
            answer = "非常抱歉, 当前数据无法回复该问题，请尝试其他问题。"
            if self._raw_answer:
                context["raw_answer"] = answer
            if self._vector_only_answer:
                context["vector_only_answer"] = answer
            if self._graph_only_answer:
                context["graph_only_answer"] = answer
            if self._graph_vector_answer:
                context["graph_vector_answer"] = answer
            return context
